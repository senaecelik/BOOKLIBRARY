...
class NetworkManager {
  ...
  NetworkManager() : _dio = Dio() {
    _dio.options.baseUrl = "https://api.orhanaydogdu.com.tr/";
  ...
  Future<Response<T>> post<T>(String path,
      {dynamic data, int timeoutSeconds = 30}) async {
    try {
  ...
  Future<Response<T>> delete<T>(String path,
      {Map<String, dynamic>? queryParameters, int timeoutSeconds = 30}) async {
    try {
  ...
  Options _getRequestOptions(int timeoutSeconds) {
    return Options(
      receiveTimeout: Duration(seconds: 60), // sunucudan yanıt bekleme süresi
    ...
    if (error is DioException) {
      if (error.response != null) {
        ...
        throw NetworkException(
            "Request failed with status code $statusCode. Response data: $responseData");
      } else {
